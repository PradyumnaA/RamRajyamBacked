// const Varat = require('../models/vratSubCategoryModel');
// const { v4: uuidv4 } = require('uuid');
// const { S3Client, PutObjectCommand } = require('@aws-sdk/client-s3');
// const VaratCounter = require('../models/vratSubCategoryCounter');
...
// controllers/vratSubCategoryController.js
...
const formatVaratResponse = async (varat) => {
    ...
    // Convert Varat SubCategory image to full URL
...
exports.createVarat = async (req, res) => {
    ...
    try {
        ...
        if (category) {
            categoryDetails = await Category.findOne({ id: category });
            if (!categoryDetails) return res.status(400).json({ error: 'Category not found' });
        }

        const imageKey = `vrat-subcategories/${uuidv4()}_${req.file.originalname}`;
